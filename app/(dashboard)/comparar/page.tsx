...
const FAZENDAS = [
  { nome: 'Fazenda Modelo', area: 5000, uf: 'MT', ref: true, esg: 68, ambiental: 64, social: 68, governanca: 71, desmat: 2800, carbono: 1240, legal: 70 },
  { nome: 'Agrícola Novo Horizonte', area: 3200, uf: 'PA', ref: false, esg: 54, ambiental: 52, social: 58, governanca: 62, desmat: 4100, carbono: 820, legal: 48 },
  { nome: 'Fazenda São Lucas', area: 7800, uf: 'GO', ref: false, esg: 79, ambiental: 82, social: 76, governanca: 80, desmat: 960, carbono: 2500, legal: 88 },
]

const COLORS = ['#16a34a', '#ef4444', '#2563eb']
const AXES = ['Ambiental', 'Social', 'Governança', 'Demat.', 'Legal']

function corEsg(v: number): string {
  return v >= 70 ? '#16a34a' : v >= 55 ? '#f59e0b' : '#ef4444'
}
function corDesmat(v: number): string {
  return v < 1500 ? '#16a34a' : v < 3000 ? '#f59e0b' : '#ef4444'
}
function corCarbono(v: number): string {
  return v >= 2000 ? '#16a34a' : v >= 1000 ? '#f59e0b' : '#ef4444'
}
...
export default function CompararPage() {
  ...
  const ROWS: { label: string; vals: number[]; corFn: (v: number) => string; suffix: string }[] = [
    { label: 'Score ESG',    vals: FAZENDAS.map(f => f.esg),       corFn: corEsg,     suffix: '/100' },
    { label: 'Ambiental',   vals: FAZENDAS.map(f => f.ambiental),  corFn: corEsg,     suffix: '/100' },
    { label: 'Social',      vals: FAZENDAS.map(f => f.social),     corFn: corEsg,     suffix: '/100' },
    { label: 'Governança',  vals: FAZENDAS.map(f => f.governanca), corFn: corEsg,     suffix: '/100' },
    { label: 'Desmat. (ha)',vals: FAZENDAS.map(f => f.desmat),     corFn: corDesmat,  suffix: '' },
    { label: 'Carbono (t)', vals: FAZENDAS.map(f => f.carbono),    corFn: corCarbono, suffix: '' },
    { label: 'Conformidade',vals: FAZENDAS.map(f => f.legal),      corFn: corEsg,     suffix: '/100' },
  ]
    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .sb-brand{display:flex;align-items:center;gap:8px;padding:16px;border-bottom:1px solid #e5e7eb;position:sticky;top:0;background:#fff;text-decoration:none}
        ...
        .comp-cell.ref-card{background:#f0fdf4}
          ...
          {MENU.map(group => (
            <div key={group.group} className="sb-group">
              ...
              {group.items.map(item => (
                <a key={item.id} href={item.href} className={`sb-item${item.id==='comparar'?' active':''}`}>
          ...
          <div className="content">
            <div className="panel">
              ...
              <div className="comp-row header">
                ...
                {FAZENDAS.map(f => (
                  <div key={f.nome} className={`comp-cell${f.ref?' ref-card':''}`} style={{fontWeight:700,fontSize:12,flexDirection:'column',alignItems:'flex-start',gap:2}}>
                    <div>{f.nome}{f.ref && <span className="ref-badge">Referência</span>}</div>
                    <div style={{fontSize:10,color:'#9ca3af',fontWeight:400}}>{f.area.toLocaleString()} ha · {f.uf}</div>
                  </div>
                ))}
              </div>
              {ROWS.map(row => (
                <div key={row.label} className="comp-row">
                  ...
                  {row.vals.map((v, i) => (
                    <div key={i} className={`comp-cell${FAZENDAS[i].ref?' ref-card':''}`}>
                      <span className="comp-val" style={{color: row.corFn(v)}}>
            ...
            <div className="panel">
              ...
              <div className="radar-wrap">
                <svg width={cx*2} height={cy*2} viewBox={`0 0 ${cx*2} ${cy*2}`}>
                  ...
                  {AXES.map((label,i) => {
                    const angle = (i/AXES.length)*Math.PI*2 - Math.PI/2
                    return <text key={i} x={cx+Math.cos(angle)*(r+18)} y={cy+Math.sin(angle)*(r+18)+4} fontSize="10" fill="#6b7280" textAnchor="middle">{label}</text>
                  })}
                  {FAZENDAS.map((f,fi) => {
